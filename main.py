...
def validar_archivo(path: str) -> tuple[bool, str]:
    ...
    if os.path.getsize(path) < 100:
        return (
            False,
            f"[ERROR] Archivo demasiado pequeño (posiblemente vacío): {os.path.basename(path)}",
        )
...
def buscar_archivos_data() -> list[str]:
    ...
    validos = [
        f for f in todos if any(k in os.path.basename(f).lower() for k in keywords)
    ]
...
def correr_parse(archivos_origen: list[str]) -> list[str]:
    ...
    for path in archivos_origen:
        ...
        print(
            f"  [OK] {total} filas | {completas} con EMA200 ({completas / total * 100:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--pasos", "-p", nargs="+", metavar="PASO", choices=PASOS_DISPONIBLES
    )
    ...
    mapa = {
        ...
        "correlation": (
            ...
            "Analisis de correlacion entre distancias",
        ),
        "scale": ("scale_analysis", "Relaciones de escala entre EMAs"),
        "compare": ("timeframe_comparison", "Comparativa entre timeframes"),
